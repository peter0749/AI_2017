...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    "*** YOUR CODE HERE ***"
    self.full_coins = tuple([1]*len(self.corners)) ## encoded state
    self.coin_collected = tuple([0]*len(self.corners))
    if self.startingPosition in self.corners:
        temp = list(self.coin_collected)
        temp[self.startingPosition]=1
        self.coin_collected = tuple(temp)
        del temp
    self.start_coin = self.coin_collected

  def getStartState(self):
    "Returns the start state (in your state space, not the full Pacman state space)"
    "*** YOUR CODE HERE ***"
    return (self.startingPosition, self.start_coin)

  def isGoalState(self, state):
    "Returns whether this search state is a goal state of the problem"
    "*** YOUR CODE HERE ***"
    return state[1]==self.full_coins
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if self.walls[nx][ny]: continue
      coin_state = state[1]
      if (nx,ny) in self.corners:
        temp = list(coin_state)
        temp[self.corners.index((nx,ny))] = 1
        coin_state = tuple(temp)
        del temp
      successors.append((((nx,ny),coin_state), action, 1))
...
def cornersHeuristic(state, problem):
  ...
  for i, v in enumerate(corners):
      if state[1][i]==1: continue ## already visited
      entered = True
      minx = min(minx, v[0])
      maxx = max(maxx, v[0])
      miny = min(miny, v[1])
